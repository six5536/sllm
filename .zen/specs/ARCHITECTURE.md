...
crates/app/smllm/src/commands/  # config, state, harness, mcp, graph command groups
...
plugin/                         # Claude Code plugin: hooks + MCP server
...
- Protocol: `bind`, `view`, `menu`, `fire`, `stop`, `prompt_submitted`
...
- `Engine` class over compiled JSON: `bind`, `view`, `events`, `fire`, `stop`, `promptSubmitted`, `exportState` / `importState`
...
One-command install of the hooks and MCP server for Claude Code.
