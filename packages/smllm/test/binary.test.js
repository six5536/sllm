...
test("selectPackage maps supported platforms", () => {
  assert.strictEqual(selectPackage("linux", "x64"), "@six5536/smllm-linux-x64");
  assert.strictEqual(selectPackage("darwin", "arm64"), "@six5536/smllm-darwin-arm64");
  assert.strictEqual(selectPackage("win32", "x64"), "@six5536/smllm-win32-x64");
});
...
test("binaryName appends .exe only on Windows", () => {
  assert.strictEqual(binaryName("linux"), "smllm");
  assert.strictEqual(binaryName("win32"), "smllm.exe");
});
...
test("resolveBinary returns the resolved path for a supported platform", () => {
  ...
  assert.strictEqual(
    ...
    "/fake/node_modules/@six5536/smllm-linux-x64/bin/smllm",
  );
  // Windows resolves the .exe inside its platform package.
  assert.strictEqual(
    resolveBinary("win32", "x64", fakeResolve),
    "/fake/node_modules/@six5536/smllm-win32-x64/bin/smllm.exe",
  );
});
...
test("resolveBinary errors clearly on unsupported platform", () => {
  assert.throws(
    ...
    /No prebuilt smllm binary for win32-arm64/,
  );
});
...
test("resolveBinary errors clearly when the platform package is missing", () => {
  ...
  assert.throws(
    ...
    /platform package "@six5536\/smllm-linux-x64" is not installed/,
  );
});
