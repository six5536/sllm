...
const PACKAGES = {
  "linux x64": "@six5536/sllm-linux-x64",
  "linux arm64": "@six5536/sllm-linux-arm64",
  "darwin x64": "@six5536/sllm-darwin-x64",
  "darwin arm64": "@six5536/sllm-darwin-arm64",
  "win32 x64": "@six5536/sllm-win32-x64",
};
...
function binaryName(platform) {
  return platform === "win32" ? "sllm.exe" : "sllm";
}
...
function resolveBinary(platform, arch, requireResolve = require.resolve) {
  ...
  if (!pkg) {
    throw new Error(
      `No prebuilt sllm binary for ${platform}-${arch}.\n` +
        `Supported platforms: ${SUPPORTED}.\n` +
        `Install from source instead: cargo install sllm`,
    );
  }
  try {
    ...
    throw new Error(
      `The sllm platform package "${pkg}" is not installed.\n` +
        `Optional dependencies were most likely skipped during install.\n` +
        `Reinstall sllm, or build from source: cargo install sllm`,
    );
  }
}
